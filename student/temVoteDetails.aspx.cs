...
{
    public partial class temVoteDetails : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                /////
                string name = Request.QueryString["sname"];
                title.Text = '"' + Request.QueryString["position"] + '"'+ name + "获票详情表";
                bind();
            }
        }
        public void bind()
        {
            ...
            string sqlstr = "select max(a.vote_id)as vote_id,max(a.position)as position,max(a.grade_id)as grade_id,a.vote_name,max(b.counts)as counts from Tx_vote as a,(select count(*) " +
                "as counts,vote_name from Tx_vote where position='" + Request.QueryString["position"] + "' and stu_id='" + Request.QueryString["sid"] + "' and caste='1' group by vote_name) " +
                "as b where a.vote_name=b.vote_name and a.position='" + Request.QueryString["position"] + "' and a.stu_id='" + Request.QueryString["sid"] + "' and a.caste='1' group by a.vote_name";
